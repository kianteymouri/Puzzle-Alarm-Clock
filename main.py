...
def load_alarms() -> list[dict]:
    ...
    Returns a list of alarm dicts:
        {"time": "07:30", "song": "morning.mp3"}
    Handles both legacy flat-list format and new dict format.
    """
...
def load_weather_temp() -> str:
    """
    Reads /home/kian/alarm_web/weather.json if present.
    Expected format:
        {"temp_f": 68}
    or
        {"temp_f": "68"}
...
def normalize_temp_value(value) -> str:
    """
    Returns a short temp string suitable for the LCD.
    Examples:
        68 -> "68"
        "72" -> "72"
        "72.4" -> "72"
...
def build_clock_lines() -> tuple[str, str]:
    """
    Top line: centered time
    Bottom line: temp + date
    Example:
        '    9:41 PM    '
        '68F 04/05/2026 '
    """
...
def api_set_weather():
    data = request.get_json(silent=True) or {}
    temp = data.get("temp_f", None)
    set_outside_temp(temp)
    return jsonify({"success": True, "temp_f": outside_temp_f})
